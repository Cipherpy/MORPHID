...
"""
Shape feature evaluation & confusion matrix

Preprocessing:
- Same as original script: extract 'Shape:' text, keep up to first comma,
  build crosstab, row-normalize.

New:
- Compute classification metrics (accuracy, precision, recall, F1, classification report)
  using the cleaned shape label pairs.
...
OUT_DIR        = "plots/"
OUT_NORM_CSV   = os.path.join(OUT_DIR, "shape_cm_normalized_llama.csv")
OUT_PNG        = os.path.join(OUT_DIR, "shape_dotmatrix_correct_vs_wrong_llama.png")  # keep name for compatibility

# Extra outputs for metrics
OUT_REPORT_TXT   = os.path.join(OUT_DIR, "shape_classification_report_llama.txt")
OUT_REPORT_CSV   = os.path.join(OUT_DIR, "shape_classification_report_llama.csv")
OUT_SUMMARY_JSON = os.path.join(OUT_DIR, "shape_summary_metrics_llama.json")
...
def pastel_bluegreen_cmap():
    """Pastel blue–green colormap similar to ViT eval script."""
    stops = [
        (0.00, "#ffffff"),
        (0.05, "#f2fdfa"),
        (0.10, "#dff8f3"),
...
def plot_shape_confusion_with_recall(
    y_true,
    y_pred,
    class_names,
    filename,
    ...
    - Rows = reference (true) shape features
    - Cols = generated (predicted) shape features
...
# Parse GT and predicted raw phrases (same logic as before)
df["shape_gt_raw"]  = df[COL_GT].apply(extract_raw_ostium)
df["shape_gen_raw"] = df[COL_PRED].apply(extract_raw_ostium)
...
# Crosstab (counts) on raw shape strings (as before)
...
y_true = filtered["shape_gt_raw"].values
y_pred = filtered["shape_gen_raw"].values

# Class names = sorted union of all labels seen in true or predicted
class_names = sorted(list(set(y_true) | set(y_pred)))

# Overall metrics (macro, similar to ViT script)
...
print("\n=== Shape Feature Metrics ===")
print(f"Top-1 Accuracy    : {acc:.4f}")
print(f"Precision (macro) : {prec:.4f}")
print(f"Recall  (macro)   : {rec:.4f}")
print(f"F1-score (macro)  : {f1:.4f}")

# Classification report (per-shape category)
...
# ===================== PLOT (NEW STYLE) ===================
plot_shape_confusion_with_recall(
    y_true=y_true,
    y_pred=y_pred,
